...
在 `tdesign-vue-next` 的 `CI` 流程中，单元测试模块的执行效率太低，每次在单元测试这个环节都需要花费 `6m` 以上。加上依赖安装，`lint` 检查等环节，需要花费 `8m` 以上。
...
因为我们老的测试方案基于 `jest`，相应的 `API` 都有。所以在迁移过程中，兼容性问题基本上只有一些从 `jest` 中的函数，切换到 `vi`，其他问题没有遇到。
