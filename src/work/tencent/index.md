...
最近把 `TDesign` 的单元测试方案迁移到了 `vitest`, 顺带做了一些优化，整个运行效率提升了 `50%`, `TD` 在单元测试这一块做的讲道理挺烂的, 我们一直在不断的增加组件库, 确实没有花很多时间在单元测试上面，这波仔细捋了捋，感觉我们这个开源项目的单元测试做的还不如我毕业半年时候做的内部组件库好。后面要开始去推动这一块的建设，尝试用TDD去做这件事件。主导和推动一件事件可以得到很大程度的成长的，希望在接下来的这段时间内在技能树中点亮单元测试这项很重要的技能。💡
